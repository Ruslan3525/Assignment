package kz.kalybayevv.Test.environments;

import kz.kalybayevv.Test.abilities.Flyable;
import kz.kalybayevv.Test.exceptions.OutOfBound;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

import java.util.ArrayList;
import java.util.List;

@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
public class Cell<T extends Flyable> extends Habitat {
    private Double size;
    private List<Flyable> objects = new ArrayList<>();

    public Cell(Double size) {
        this.size = size;
    }

    public <T extends Flyable> String addObj(T obj) throws OutOfBound {
        this.size = getSize() - obj.getComfortableSpace();
        if (size < 0) {
            throw new OutOfBound("Not have enough space for your bird");
        }
        objects.add(obj);
        return "Your bird is delighted";
    }
}
